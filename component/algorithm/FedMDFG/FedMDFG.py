...
import torch
import time
from component.algorithm.common.utils import get_fedmdfg_d


class FedMDFG(cn.Algorithm):
    def __init__(self,
                 name='FedMDFG',
                 data_loader=None,
                 loader_name=None,
        ...
        super().__init__(name, data_loader, loader_name, module, device, train_setting, client_num, client_list, online_client_num, metric_list,
                         max_comm_round, epochs, save_name, outFunc, write_log, params)
    ...
    def train_a_round(self):
        ...
        client_id_list = self.get_client_attr('id')  
        force_active = False
        increase_count = 0
        ...
        d, vec, _, _ = get_fedmdfg_d(g_locals, l_locals, add_grads, self.theta, prefer_vec, force_active, self.device)
        ...
        self.last_client_id_list = self.get_client_attr('id')
