...
def outFunc(alg):
    ...
    for i, metric_history in enumerate(alg.comm_log['client_metric_history']):
        training_loss = metric_history['training_loss'][-1] # 原本为training_loss
    ...
    stream_log += f'Training Loss List: {[f"{x:.3f}" for x in loss_list]}\n'
    stream_log += f'Average: {format(np.mean(loss_list), ".3f")}. Variance: {format(np.var(loss_list), ".3f")}. ' + '\n'
    stream_log += '\n'
...
def read_params():
    ...
    parser.add_argument('--module', help='module name;',
                        type=str, default='LinearRegression')
    ...
    parser.add_argument('--dataloader', help='dataloader name;',
                        type=str, default='DataLoader_synthetic')
    ...
    parser.add_argument('--N', help='client num', type=int, default=2)
    ...
    parser.add_argument('--test_interval',
                        help='test interval', type=int, default=500) # default=1
    ...
    parser.add_argument('--sgd_step', help='sgd training',
                        type=str, default='False') # 是否SGD的逻辑值，default='False'
    ...
    parser.add_argument(
        '--lam', help='parameter tau in Ditto', type=float, default=0.8) #default=0.1

    parser.add_argument(
        '--epsilon', help='parameter epsilon in FedMGDA+', type=float, default=0.1)
    # ***************
    parser.add_argument(
        '--theta', help='parameter theta in FedMDFG', type=float, default=11.25)
    parser.add_argument(
        '--s', help='parameter s in FedMDFG', type=float, default=5)
    parser.add_argument(
        '--pow', help='parameter pow in AdaFed', type=float, default=3)
    parser.add_argument(
        '--prefer', help='parameter prefer in FairPD & FairMO', type=float, default=6e-3)
    # ***************
...
def initialize(params):
    ...
    )), lr=params['lr']) # no weight_decay
    # torch.nn.CrossEntropyLoss()
    train_setting = {'criterion': torch.nn.MSELoss(),
    ...
    algorithm.save_folder = data_loader.nickname
